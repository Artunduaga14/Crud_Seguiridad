...
namespace Web.Controllers
{
    ...
    public class PermissionController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateRol([FromBody] PermissionDto PermissionDto)
        {
            try
            {
                var createdRol = await _PermissionBusiness.CreatePermissionAsync(PermissionDto);
                return CreatedAtAction(nameof(GetPermissionById), new { id = createdRol.Id }, createdRol);
            }
            catch (ValidationException ex)
            {
